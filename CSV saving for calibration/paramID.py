...
mcmc_lib = 'emcee' # TODO make this a user variable, this is a mcmc library ensemble sued to sample parameter space, i.e the walker
if mcmc_lib == 'emcee':
    import emcee
elif mcmc_lib == 'zeus':
    import zeus
...
class CVS0DParamID():
    ...
    def __init__(self, model_path, model_type, param_id_method, mcmc_instead, file_name_prefix,
        ...
        self.param_id_obs_file_prefix = re.sub('.json', '', os.path.split(param_id_obs_path)[1]) # this fetches info from the obs_data.json file
    ...
    def compute_cost_for_param_vec(self, param_vec):
        """
        Overwrite internal parameters with param_vec, run one simulation,
        then return the total Gaussian‐MLE cost (exactly as simulate_with_best_param_vals would).
        """
        # 1) Tell the underlying OpencorParamID or OpencorMCMC to use these params:
        self.param_id.set_best_param_vals(param_vec)

        # 2) Force a single simulation with these params (the same way simulate_with_best_param_vals does):
        #    (NOTE: simulate_with_best_param_vals just calls param_id.simulate_once internally.)
        self.param_id.simulate_once(reset=True, only_one_exp=-1)

        # 3) Now ask for cost + list of observables at “best_param_vals”:
        total_cost, _ = self.param_id.get_cost_and_obs_from_params(param_vec)
                    ...
                    if not this_obs_waveform_plotted:
                        ...
                        if not self.obs_info["data_types"][II] == 'frequency':
                            ...
                            for temp_sub_idx in range(self.protocol_info["num_sub_per_exp"][exp_idx]):
                                ...
                                # ---- SAVE TO CSV ----
                                csv_path = os.path.join(self.plot_dir,
                                    f'{obs_name_for_plot}_exp{exp_idx}_sub{temp_sub_idx}.csv')
